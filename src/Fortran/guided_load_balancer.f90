module SLB4MPI_guided_load_balancer_m
  ...
  public :: guided_load_balancer_t

contains
  !>
  !> @brief initialize guided load balancer
  !>
  ...
  !>
  !> @note `upper_bound` can be only less than or equal `guided_load_balancer_t%upper_bound`.
  ...
  logical function get_range(lb, lower_bound, upper_bound) result(to_compute)
    ...
    integer(8) :: associated_tasks, root_counter
    ...
    associated_tasks = min(max(lb%upper_bound - lb%counter, lb%min_chunk_size), lb%max_chunk_size)
    lb%counter = lb%counter + associated_tasks
#endif

    upper_bound = min(lower_bound + associated_tasks - 1, lb%upper_bound)
  ...
  !>
  !> @brief guided load balancer destructor
  !>
  !> @param[in,out] lb - load balancer object
  !>
  subroutine clean(lb)
